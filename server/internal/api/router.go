...
import (
	...
	"github.com/hungnm98/seshat-server/internal/api/middleware"
...
func NewRouter(ctx context.Context, deps Dependencies) *gin.Engine {
	...
	projectRoutes.GET("/graph/dependencies", func(c *gin.Context) {
		...
		result, ok, err := deps.Query.FileDependencyGraph(c.Request.Context(), c.Param("projectID"), filePath, depth)
		if err != nil {
			c.JSON(http.StatusBadRequest, gin.H{"error": err.Error()})
			return
		}
		...
		c.JSON(http.StatusOK, gin.H{"graph": result, "generated_at": time.Now().UTC()})
	})
