...
func (s *Store) FileDependencyGraph(ctx context.Context, projectID, filePath string, depth int) (model.FileDependencyGraph, bool, error) {
	...
	s.mu.RLock()
	defer s.mu.RUnlock()
	graph, found := storage.BuildFileDependencyGraph(
		version,
		append([]model.File(nil), s.filesByVersion[version.ID]...),
		append([]model.Symbol(nil), s.symbolsByVersion[version.ID]...),
		append([]model.Relation(nil), s.relationsByVersion[version.ID]...),
		filePath,
		depth,
	)
