...
func (s *Store) FileDependencyGraph(ctx context.Context, projectID, filePath string, depth int) (model.FileDependencyGraph, bool, error) {
	version, ok, err := s.LatestProjectVersion(ctx, projectID)
	if err != nil || !ok {
		return model.FileDependencyGraph{}, false, err
	}
	files, err := s.listFilesByVersion(ctx, version.ID)
	if err != nil {
		return model.FileDependencyGraph{}, false, err
	}
	symbols, err := s.listSymbolsByVersion(ctx, version.ID)
	...
	graph, found := storage.BuildFileDependencyGraph(version, files, symbols, relations, filePath, depth)
...
func (s *Store) listFilesByVersion(ctx context.Context, versionID string) ([]model.File, error) {
	rows, err := s.db.QueryContext(ctx, `
		select id, path, language, checksum
		from files
	...
	for rows.Next() {
		var file model.File
		if err := rows.Scan(&file.ID, &file.Path, &file.Language, &file.Checksum); err != nil {
			return nil, err
		}
